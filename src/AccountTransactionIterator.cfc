...
<cfcomponent displayname="cfRecurly.AccountTransactionIterator" output="false">
    ...
    <cfset Variables.objTransaction = createObject( "Transaction" )>
    ...
    <cffunction name="init" access="public" output="true" hint="Constructor">
        ...
        <cfargument name="stateFilter" required="false" default="" type="string" hint="successful|failed|voided">
        <cfargument name="typeFilter" required="false" default="" type="string" hint="authorization|refund|purchase">
        ...
        <cfset Variables.strTypeFilter = Arguments.typeFilter>
    ...
    <cffunction name="next"
        ...
        <cfelse>
            <cfif Variables.Util.areFieldsEmpty( Variables.stLastReponse )>
                <cfset stAPICall = Variables.API.get("accounts/#accountId#/transactions", { "state" = Variables.strStateFilter, "type" = Variables.strTypeFilter, "per_page" = Variables.iPerPage })>
            <cfelse>
                ...
                <cfif len( strLink )>
                    ...
                    <cfset stAPICall = Variables.API.get("accounts/#accountId#/transactions", { "state" = Variables.strStateFilter, "type" = Variables.strTypeFilter, "per_page" = Variables.iPerPage, "cursor" = cursor })>
                </cfif>
            </cfif>
        </cfif>
        ...
        <cfif len( stAPICall["data"] )>
            ...
            <cfif isDefined( "xoResultNode.transactions" )>
                <cfset var xoTransactions = xoResultNode.transactions>
                <cfloop index="idx" from="1" to="#ArrayLen(xoTransactions.XmlChildren)#">
                    <cfset var xoTransaction = xoTransactions.XmlChildren[ idx ]>
                    <cfset var stTransaction = Variables.objTransaction.parseXML( toString( xoTransaction ) )>
                    <cfset stTransaction["account_id"] = accountId>
                    <cfset arrayAppend( arrRet, new Transaction( Variables.API, stTransaction ) )>
                </cfloop>
            </cfif>
        </cfif>
    ...
    <cffunction name="reset" access="public" output="false" returntype="AccountTransactionIterator">
